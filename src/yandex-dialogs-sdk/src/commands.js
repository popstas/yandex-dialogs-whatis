...
class Commands {
  ...
  search(requestedCommandName) {
    let matchedStrings = this._searchStrings(requestedCommandName)
    const matchedRegexps = this._searchRegexps(requestedCommandName)
    if (matchedStrings.length > 0) {
      const bestScore = matchedStrings[0].score;
      const scoreThreshold = 2;
      matchedStrings = matchedStrings.map(answer => {
        return {
          ...answer.item,
          ...{
            score: answer.score,
      ...
      if (matchedStrings.filter(answer => !answer.minor).length > 1) {
        // возможно неточный ответ
      }
